#!/bin/bash
#SBATCH -A lrn087
#SBATCH -J lumina-100M-al-2node
#SBATCH -o smoke-100M-al-2node-%j.log
#SBATCH -e smoke-100M-al-2node-%j.log
#SBATCH -p batch
#SBATCH -q normal
#SBATCH -t 00:30:00
#SBATCH -N 2
...
export LUMINA_LOGGING_DIR=/lustre/orion/lrn087/proj-shared/mlupopa/logs/lumina-100M-al-2node-${SLURM_JOB_ID}
export LUMINA_CHECKPOINT_DIR=/lustre/orion/lrn087/proj-shared/mlupopa/checkpoints/lumina-100M-al-2node-${SLURM_JOB_ID}
export LUMINA_CONFIG=configs/config.frontier.rocm713.al.smoke.100M.yaml
export LUMINA_MODEL_CONFIG=configs/model/heterognn_100M.yaml
export LUMINA_CASES="case2000"
export LUMINA_GROUP_IDS="0"
...
# 2-node validation of the NCCL_NET_PLUGIN=none inter-node TCP fallback.
# NCCL_DEBUG=INFO here (vs WARN in production) so we can see which transport
# RCCL selects between nodes and confirm the collectives complete without the
# illegal-instruction / CXI ENOSYS failures seen with the aws-ofi-nccl plugin.
