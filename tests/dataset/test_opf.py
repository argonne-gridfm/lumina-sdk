...
def _require_snapshot_fixtures():
    for path in (_EXAMPLE_JSON_PATH, _PROCESS_JSON_SNAPSHOT_PATH):
        if not osp.exists(path):
            pytest.skip(f"snapshot fixture not in repo: {path}")


def test_process_json_file_example_0_matches_snapshot():
    _require_snapshot_fixtures()
...
def test_opf_on_disk_dataset_processes_json_file_like_process_json_file(tmp_path):
    if not osp.exists(_ON_DISK_SNAPSHOT_PATH):
        pytest.skip(f"snapshot fixture not in repo: {_ON_DISK_SNAPSHOT_PATH}")
...
def test_opf_dataset_loads_case14_and_exposes_expected_schema(tmp_path):
    """End-to-end smoke test mirroring the quickstart snippet: construct an
    ``OPFDataset`` for case14 group 0 and verify the documented sample count
    and hetero schema. Downloads from Google Cloud Storage on first run
    (~38s); subsequent runs within the same tmp_path reuse the processed
    cache."""
    dataset = OPFDataset(
        root=str(tmp_path),
    ...
    sample = dataset[0]
    assert set(sample.node_types) == {'bus', 'generator', 'load', 'shunt'}
    assert set(sample.edge_types) == {
        ('bus', 'ac_line', 'bus'),
        ('bus', 'transformer', 'bus'),
