"""
Homogeneous Graph Neural Network models for ACOPF benchmarking.
...
from .utils import from_adj_to_edge_index_torch


def get_gnnNets(input_dim, output_dim, model_params):
    """ Get the GNN model based on the model name.
...
def identity(x: torch.Tensor, batch: torch.Tensor):
    """Identity function for readout layer.
...
def cat_max_sum(x, batch):
    """Concatenate max and sum of node embeddings.
...
def get_readout_layers(readout):
    """Get the readout layer based on the readout name.

    Args:
        readout (str): Readout layer name.

    Returns:
        function: Readout layer function.
    """
    readout_func_dict = {
...
class GNNPool(nn.Module):
    """Graph pooling module for readout operations."""
...
class GNNBase(nn.Module):
    """ Homogeneous GNN Base model for all GNN models.
